...
def sents_to_vec(vec_dict, sentences):
    ...
    for sent in sentences:
        ...
        for word in sent["word"]:
            if word in vec_dict:
                sample.append(vec_dict[word])

            elif word[0:min(len(word),6)] in vec_dict:
                sample.append(vec_dict[word[0:min(len(word),6)]])

            elif word[0:min(len(word),5)] in vec_dict:
                sample.append(vec_dict[word[0:min(len(word),5)]])

            elif word[0:min(len(word),4)] in vec_dict:
                sample.append(vec_dict[word[0:min(len(word),4)]])
            else:
                out_dict += 1
                sample.append([0 for i in range(100)])


        embeddings.append(sample)
    if out_dict:
            print(out_dict,' out of',len(vec_dict.keys()))
...
def read_conll(fname, max_len=1e3,rm_null=True, prc_num=True):
    ...
    loc = 0
    codigo_sentence = ''
    codigo_aceitos_sentence = []
    with open(fname) as fin:
        for line in fin:
           
            if line != '\n':
                line = line.strip().split('\t')
                if line[0][0] == '#' or '-' in line[0]:
                  
                  if 'sent_id = ' in line[0]:
                     codigo_sentence = line[0].split('=')[1]
                  continue
                sent["head"].append((int(line[0]), 
                    cast_to_int(line[6])))#X#cast_to_int(line[3])))
                if rm_null and line[2] == '-NONE-':
                    null_sent.append(loc)
                else:
                    sent["tag"].append(line[3])#X#sent["tag"].append(line[2])
                    if prc_num and is_number(line[1]):
                        sent["word"].append('0')
                    else:
                        sent["word"].append(line[1].lower())#X#line[1])

                loc += 1
            else:
                loc = 0
                if len(sent) > 0 and len(sent) <= max_len:
                    codigo_aceitos_sentence.append(codigo_sentence)
    ...
    return sentences,codigo_aceitos_sentence, null_total
...
def to_input_tensor(sents, pad, device):
    ...
    print('TYPE',type(sents))
    print('TYPE',type(sents[0][0]))
    print(sents[0][0])
    sents_t = torch.tensor(np.array(sents), dtype=torch.float32, requires_grad=False, device=device)
